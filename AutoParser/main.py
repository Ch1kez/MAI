    ...
    while True:
        try:
            ...
            while count < 3:
                ...
                cur = base.cursor()
                base.execute('CREATE TABLE IF NOT EXISTS TABLE' + str(
                    count) + '(Id, Array_photo_links, Model, Badge, FuelType, FormYear, Mileage, OfficeCityState, Price, Manufacturer)')
                base.commit()
                ...
                while count_for_photo < len(data['SearchResults'][0]['Photos']):
                    Array_photo_links += ['http://ci.encar.com/carpicture' + str(
                        data['SearchResults'][0]['Photos'][count_for_photo][
                            'location']) + '?impolicy=heightRate&rh=91&cw=122&ch=91&cg=Center&wtmk=http://ci.encar.com/wt_mark/w_mark_03.png&wtmkg=SouthEast&wtmkw=45&wtmkh=12.3']
                    count_for_photo += 1
                ...
                if Last_id_from_DB == Id:
                    ...
                    if int(max_size_db) > 49:
                        ...
                        while id < len(id_for_message):
                            await bot.send_message(id_for_message[id][0],
                                                   'Новая партия информации по машинам на странице ' + str(count))
                            await bot.send_document(id_for_message[id][0],
                                                    document=open('DB' + str(count) + '.db', 'rb'))
                            id += 1
                        ...
                        base.execute('CREATE TABLE IF NOT EXISTS TABLE' + str(
                            count) + '(Id, Array_photo_links, Model, Badge, FuelType, FormYear, Mileage, OfficeCityState, Price, Manufacturer)')
                        base.commit()
                    cur.execute('INSERT INTO TABLE' + str(count) + ' VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (
                    Id, Array_photo_links, Model, Badge, FuelType, FormYear, Mileage, OfficeCityState, Price,
                    Manufacturer))
                    base.commit()
                    count += 1
            print('##########################################')
